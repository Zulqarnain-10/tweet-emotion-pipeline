...
Held-out test split, n = [todo], share labelled [todo] [todo], decision threshold [todo]. Nothing below was tuned on it.

| Metric | [todo] (this model) | Majority baseline |
|---|---|---|
| Accuracy | [todo] | [todo] |
| Precision | [todo] | |
| Recall | [todo] | |
| F1 | [todo] | |
| ROC-AUC | [todo] | |
| PR-AUC (average precision) | [todo] | |
| Brier score (lower is better) | [todo] | |
| Log loss (lower is better) | [todo] | |

The majority baseline predicts the more common class for every tweet, so only its accuracy is meaningful.

Candidates, [todo]-fold stratified cross-validation on the training split only, selected by cv [todo] (models/version.json):

| Candidate | cv ROC-AUC (mean ± std) | cv accuracy | Fit seconds | Shipped |
|---|---|---|---|---|
| [todo] | [todo] | [todo] | [todo] | |

Latency: p95 [todo] ms for POST /predict, [todo] requests at concurrency [todo], [todo] requests per second, on [todo] (reports/loadtest.json).

Provenance: trained [todo], git [todo], data sha256 [todo], rows [todo] train / [todo] test (models/version.json).
<!-- metrics:end -->
